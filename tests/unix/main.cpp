...
TEST_CASE("Workers tests", "[tools][workers]") {
  ...
  SECTION("Should process the enqueued jobs correctly") {
    ...
    display("[Workers] Expected warning passing nullptr");
    workers.enqueue_job(
        []() { std::cout << logger::debug() + "processing: job 1\n"; });
    workers.enqueue_job(
        []() { std::cout << logger::debug() + "processing: job 2\n"; });
    workers.enqueue_job(
        []() { std::cout << logger::debug() + "processing: job 3\n"; });
    workers.enqueue_job(
        []() { std::cout << logger::debug() + "processing: job 4\n"; });
    workers.enqueue_job(
        []() { std::cout << logger::debug() + "processing: job 5\n"; });
...
TEST_CASE("TCP socket tests server operations",
  ...
  SECTION("Should assign a name to the socket") {
    ...
    THEN("tryng to bind the socket again should diplay a warning message") {
      ...
      display("[TCP socket] Expected warning, name already assigned.");
    }
    ...
    std::thread thread_server([&server] {
      ...
      server.on_connection([](const std::shared_ptr<tcp::client> &client) {
        client->async_receive(1024, [](bool success, std::vector<char> data) {
          if (success) {
            std::cout << logger::debug() + data.data();
          }
        });
      });
